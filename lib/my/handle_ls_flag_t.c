...
static int t_check_ls_root(int argc, const char **argv)
{
    int check = 1;
...
static int find_files_nbr(const char *argv)
{
...
static char **find_files_tab(const char *argv)
{
    DIR *dir = opendir(argv);
    struct dirent *entry;
    int files_nb = find_files_nbr(argv);
...
static int find_index(char **list, const char *path)
{
    struct stat ls_info;
...
void remove_index(char **list, int index)
{
    free(list[index]);
...
void sort_list_t(const char *path)
{
    char **list = find_files_tab(path);
    int index;
...
int t_arguments(int argc, const char **argv, int i)
{
    DIR *dir;
    struct dirent *entry;
    struct stat ls_info;
